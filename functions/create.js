// POST /create - Créer une page affiliée
export async function onRequestPost(context) {
  ...
  try {
    ...
    if (!env.AFFILIATE_SITES) {
      return new Response(JSON.stringify({ error: 'Base de données non disponible' }), {
    ...
    await env.AFFILIATE_SITES.put(id, JSON.stringify({
      html, affiliateLink, productName, productImage,
    ...
    return new Response(JSON.stringify({ success: true, url: siteUrl, id }), {
      headers: corsHeaders
    });
  } catch (error) {
