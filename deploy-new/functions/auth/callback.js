...
export async function onRequestGet(context) {
  ...
  try {
    ...
    if (!pages || pages.length === 0) {
      return Response.redirect(redirectUrl + '?fb_error=' + encodeURIComponent('Aucune Page Facebook trouvee'), 302);
    }

    // Parse state to get siteId and productName
    ...
    if (pages.length === 1) {
      ...
      return Response.redirect(
        redirectUrl + '?fb_connected=true&page_name=' + encodeURIComponent(page.name),
        302
      );
    }

    // Multiple pages - show selection page with proper encoding
    return new Response(getSelectionHtml(pages, siteId, productName, redirectUrl), {
...
async function savePageToken(env, siteId, pageId, pageName, pageToken, productName) {
  // Store in KV with siteId as key
  ...
  // Also add to the list of all connected pages for cron job
  ...
  // Remove existing entry for this siteId if exists
...
function getSelectionHtml(pages, siteId, productName, redirectUrl) {
  // Store pages data in a script tag as JSON to avoid escaping issues
...
<body>
  <div class="container">
    ...
    <p class="subtitle">Choisissez la Page Facebook sur laquelle publier automatiquement</p>
    <div id="pagesList"></div>
    <div id="loading" class="loading">Connexion en cours...</div>
  </div>
  
  <script>
    // Pages data embedded as JSON
    ...
    pagesData.forEach((page, index) => {
      ...
      btn.innerHTML = \`
        <img src="\${page.picture?.data?.url || 'https://via.placeholder.com/50'}" alt="\${page.name}">
        <div>
